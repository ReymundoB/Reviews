...
async def create_review(user_review:ReviewRequestModel):
    ...
    if Movie.select().where(Movie.id==user_review.movie_id).first() is None:
        raise HTTPException(status_code=404,detail='La pelicula no fue encontrada')
...
async def get_review(review_id:int):
    ...
    if user_review is None:
        raise HTTPException(status_code=404, detail='La reseña no se encuentra')
...
async def update_review(review_id:int, review_request:ReviewRequestPutModel):
    ...
    if user_review is None:
        raise HTTPException(status_code=404, detail='La reseña no fue encontrada')
